"""Shared idea backfill logic — finds narratives with < 3 ideas and generates more via LLM.

Used by:
  - app/schedulers/scheduler.py (hourly cron)
  - app/schedulers/web_job.py (post-scrape Step 4)
  - app/schedulers/twitter_job.py (post-scrape Step 4)
  - scripts/backfill_ideas.py (CLI)
"""
...
async def backfill_narrative(db: AsyncSession, narrative: Narrative, dry_run: bool = False) -> int:
...
async def find_underpopulated_narratives(db: AsyncSession) -> list[Narrative]:
    """Return active narratives that have fewer than MIN_IDEAS ideas."""
    idea_count_sq = (
        select(
            Idea.narrative_id,
...
async def run_idea_backfill(db: AsyncSession, dry_run: bool = False) -> dict:
    """Find all under-populated narratives and backfill ideas. Returns summary dict."""
    narratives = await find_underpopulated_narratives(db)

    if not narratives:
        logger.info("[BACKFILL] All narratives have >= 3 ideas, nothing to do")
        return {"narratives_processed": 0, "ideas_created": 0}
    ...
    summary = {"narratives_processed": len(narratives), "ideas_created": total_created}
