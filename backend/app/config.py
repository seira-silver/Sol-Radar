...
class Settings(BaseSettings):
    ...
    # Scheduler
    WEB_SCRAPE_INTERVAL_HOURS: int = 10  # every 3 hours
    TWITTER_SCRAPE_INTERVAL_HOURS: int = 24  # every 24 hours
    NARRATIVE_SYNTHESIS_INTERVAL_DAYS: int = 1
    # New source-specific schedulers
    COINGECKO_SCRAPE_INTERVAL_HOURS: int = 10  # trending coins can change quickly
    DUNE_SCRAPE_INTERVAL_HOURS: int = 10  # on-chain trend queries are heavier
    GITHUB_SCRAPE_INTERVAL_HOURS: int = 12  # repo activity does not need hourly polling
